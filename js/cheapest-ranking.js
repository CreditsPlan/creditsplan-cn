/**
 * cheapest-ranking.js — 最便宜套餐榜交互增强
...
// ---- 列定义 ----
const COLUMNS = [
  { key: 'rank', label: '#', sortable: true, filterable: false, numeric: true },
  { key: 'brand', label: '品牌', sortable: true, filterable: true },
  { key: 'plan', label: '套餐', sortable: true, filterable: true },
  { key: 'price', label: '月费', sortable: true, filterable: true, numeric: true },
  { key: 'model', label: '代表模型', sortable: true, filterable: true },
  { key: 'verified', label: '核对日期', sortable: true, filterable: true }
];
...
function renderTable(wrap, allRows) {
  ...
  tbody.innerHTML = filtered.map(row => {
    const brandCell = row.cells.brand || '';
    const planCell = row.cells.plan || '';
    // 保留原始 HTML（含链接）——从原始 tr 读取
    ...
    return `<tr>
      ...
      <td data-cheapest-col="brand" data-sort-value="${escapeHtml(row.cells.brand)}" class="py-2 pr-3">${tdMap.brand ? tdMap.brand.innerHTML : escapeHtml(brandCell)}</td>
      <td data-cheapest-col="plan" data-sort-value="${escapeHtml(row.cells.plan)}" class="py-2 pr-3">${tdMap.plan ? tdMap.plan.innerHTML : escapeHtml(planCell)}</td>
  ...
  if (countEl) {
    countEl.textContent = `显示 ${filtered.length} / ${allRows.length} 个套餐`;
  }
}
...
function renderHeaderCell(col, allRows) {
  ...
  let filterMenu = '';
  if (col.filterable) {
    const options = getFilterOptions(allRows, col.key);
    filterMenu = `
  ...
  return `
    <th scope="col" class="plan-column-filter px-3 py-3 text-left font-semibold text-slate-900 dark:text-white">
      <button type="button" class="plan-column-filter-trigger${isActive || hasFilter ? ' is-active' : ''}" data-cheapest-sort="${col.key}" title="点击排序${col.filterable ? '，点击下拉箭头筛选' : ''}">
