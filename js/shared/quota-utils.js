// shared/quota-utils.js — 套餐额度解析与等效单价计算
// 供「帮我选套餐」计算器（plan-advisor.js）与套餐对比表格（plans-table.js）复用。
// 额度字段为自由文本（次 / Credits / 积分 / tokens 混合），解析时只做数字提取，
// 等效单价仅在可可靠归一为「月请求数」时计算，避免跨单位误导对比。
...
// 等效单价：有效月价 ÷ 月额度 × 1000（每千次请求成本，折算人民币）。
// 仅当月额度可可靠归一时返回 { text, estimated, basis }，否则返回 null。
export function planUnitPriceDisplay(plan) {
  ...
  const per1k = (price.cny / quota.value) * 1000;
  return {
    text: `≈¥${formatUnitPrice(per1k)}/千次`,
