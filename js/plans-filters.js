...
export const PLAN_TABLE_FILTER_COLUMNS = [
  { key: 'provider', label: '品牌', width: 7, minWidth: 150, defaultVisible: true, value: plan => displayNameForProvider(plan.provider) || EMPTY_TABLE_VALUE },
  { key: 'name', label: '套餐名称', width: 14, defaultVisible: true, value: plan => cleanValue(plan.name) || EMPTY_TABLE_VALUE },
  { key: 'monthlyPrice', label: '连续包月', width: 8, defaultVisible: true, value: plan => planTablePriceValue(plan.monthlyPrice) },
  { key: 'quarterlyPrice', label: '连续包季', width: 8, defaultVisible: true, value: plan => planTablePriceValue(plan.quarterlyPrice) },
  { key: 'annualPrice', label: '连续包年', width: 8, defaultVisible: true, value: plan => planTablePriceValue(plan.annualPrice) },
  { key: 'billingUnit', label: '计费单位', width: 8, defaultVisible: false, value: plan => planBillingUnitLabel(plan) },
  { key: 'quota', label: '额度', width: 12, defaultVisible: true, value: plan => planQuotaDisplay(plan)?.text || EMPTY_TABLE_VALUE },
  { key: 'model', label: '代表模型', width: 8, defaultVisible: true, value: plan => supportedModelDisplay(plan) || EMPTY_TABLE_VALUE },
  { key: 'dataTraining', label: '数据训练', width: 8, defaultVisible: false, value: plan => planDataTrainingLabel(plan) },
  { key: 'verifiedAt', label: '核对日期', width: 8, defaultVisible: false, value: plan => cleanValue(plan.lastVerifiedAt) || '待核对' },
  { key: 'status', label: '状态', width: 6, minWidth: 84, defaultVisible: true, value: plan => cleanValue(plan.statusLabel) || EMPTY_TABLE_VALUE }
];
...
const PLAN_TABLE_MIN_WIDTH = 1440;       // 全部列展开时的最小表格宽度（与历史 colgroup 总宽一致）
...
export function renderPlanColumnSettings() {
  ...
  return `
    <div class="plan-column-settings">
      <button type="button" class="plan-column-settings-trigger" data-plan-column-settings aria-haspopup="menu" aria-expanded="${planColumnSettingsOpen ? 'true' : 'false'}" title="选择表格显示的列">
        <span class="plan-column-settings-label">列设置${hiddenCount ? ` · 隐藏 ${hiddenCount} 列` : ''}</span>
        <span class="plan-column-filter-caret" aria-hidden="true"></span>
      </button>
      <div class="plan-column-settings-menu" role="menu"${planColumnSettingsOpen ? '' : ' hidden'}>
        ${PLAN_TABLE_FILTER_COLUMNS.map(column => `
          <button type="button" class="plan-column-settings-option${isPlanColumnVisible(column.key) ? ' is-active' : ''}" role="menuitemcheckbox" aria-checked="${isPlanColumnVisible(column.key) ? 'true' : 'false'}" data-plan-column-toggle="${escapeHtml(column.key)}">
            ...
            <span class="plan-column-settings-option-label">${escapeHtml(column.label)}</span>
          </button>
        `).join('')}
        <button type="button" class="plan-column-settings-reset" data-plan-column-reset>恢复自动</button>
      </div>
    </div>
  `;
}
...
export function syncPlanColumnSettingsMenu() {
  document.querySelectorAll('.plan-column-settings').forEach(container => {
    ...
    container.querySelectorAll('[data-plan-column-settings]').forEach(button => {
      ...
      if (!label) return;
      const hiddenCount = PLAN_TABLE_FILTER_COLUMNS.length - visiblePlanTableColumns().length;
      label.textContent = `列设置${hiddenCount ? ` · 隐藏 ${hiddenCount} 列` : ''}`;
    });
  });
}
