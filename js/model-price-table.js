...
// 与套餐表格一致：每个厂商分组默认只展示 2 条模型，点击分组头展开/收起
const MODEL_TABLE_GROUP_PREVIEW = 2;

// Anthropic 官网当前列在“旧版模型”下的型号。
...
function formatTokenPrice(value, currency) {
  ...
  return `${symbol}${n.toLocaleString('zh-CN', { maximumFractionDigits: 4 })}`;
}
...
function providerDisplayName(provider) {
  return PROVIDER_NAME_MAP[provider] || provider || '未知';
}
...
function providerIcon(provider, iconUrl) {
  ...
  return renderBrandIcon(url, providerDisplayName(provider), 'brand-icon brand-icon--tab');
}
...
const SORT_KEYS = {
  ...
  name: { label: '模型', numeric: false },
  provider: { label: '品牌', numeric: false },
  context: { label: '上下文', numeric: true, raw: m => numberOrNull(m.raw?.context_length) },
  input: { label: '输入（官方价）', numeric: true, raw: m => numberOrNull(m.raw?.input_price) },
  output: { label: '输出（官方价）', numeric: true, raw: m => numberOrNull(m.raw?.output_price) }
};
...
function sortModels(models, sortKey, sortDir) {
  ...
  sorted.sort((a, b) => {
    ...
    if (config.numeric) {
      ...
      cmp = (a.modelName || '').localeCompare(b.modelName || '', 'zh-CN');
    } else if (sortKey === 'provider') {
      cmp = providerDisplayName(a.vendor).localeCompare(providerDisplayName(b.vendor), 'zh-CN');
    }
...
// 按厂商分组；分组顺序沿用传入列表（全局排序后首次出现的顺序），组内保持全局排序
function groupModelsByProvider(models) {
  const map = new Map();
  for (const model of models) {
    const name = providerDisplayName(model.vendor);
    let group = map.get(name);
    if (!group) {
      group = { name, vendor: model.vendor, icon: model.providerIconUrl, models: [] };
      map.set(name, group);
    }
...
function renderGroupSummary(group) {
  ...
  if (cheapest) {
    ...
    parts.push(`输入 ${symbol}${cheapest.value.toLocaleString('zh-CN', { maximumFractionDigits: 4 })} 起`);
  }
  // 与套餐表格的「N 个可用」同款数量统计
  parts.push(`${group.models.length} 个模型`);
...
const FILTER_COLUMNS = {
  ...
  provider: m => providerDisplayName(m.vendor),
...
const FILTER_EMPTY_VALUES = new Set(['—', '待更新']);
const filterCollator = new Intl.Collator('zh-CN', { numeric: true, sensitivity: 'base' });
...
function filterColumnOptions(models, key) {
  ...
  return Array.from(counts.entries())
    ...
    .sort((a, b) => {
      const aEmpty = FILTER_EMPTY_VALUES.has(a.value);
      const bEmpty = FILTER_EMPTY_VALUES.has(b.value);
      if (aEmpty !== bEmpty) return aEmpty ? 1 : -1;
      return filterCollator.compare(a.value, b.value);
    });
}
...
function renderFilterMenu(key, filterState, baseModels) {
  ...
  return `<button type="button" class="plan-column-filter-trigger model-price-filter-trigger${isActive ? ' is-active' : ''}" data-model-filter-column="${key}" aria-haspopup="menu" aria-expanded="false" title="筛选${escapeHtml(config.label)}">
...
function renderTableHeader(sortKey, sortDir, filterState, baseModels) {
  const th = (key, cls = '') => {
    ...
    return `<th class="model-price-th plan-column-filter ${cls}" data-sort-key="${key}" role="columnheader" aria-sort="${active ? (sortDir === 'asc' ? 'ascending' : 'descending') : 'none'}" tabindex="0">
      <span class="model-price-th-inner">${config.label}${renderSortIcon(key, sortKey, sortDir)}</span>
...
function renderTableRow(model) {
  ...
  const displayName = providerDisplayName(model.vendor);
  const docsUrl = model.sourceUrl || model.raw?.docs_url || '';
  const legacyBadge = isLegacyModel(model)
    ? '<span class="model-price-legacy-badge">旧版</span>'
    : '';
  ...
  const providerInner = `${providerIcon(model.vendor, model.providerIconUrl)}<span>${escapeHtml(displayName)}</span>`;
  ...
  return `<tr class="model-price-row">
    ...
    <td class="model-price-td model-price-td--price">
      ...
      ${inputRaw != null ? '<span class="model-price-unit">/百万tokens</span>' : ''}
    </td>
    <td class="model-price-td model-price-td--price">
      <span class="model-price-value${outputRaw == null ? ' model-price-value--empty' : ''}">${escapeHtml(outputDisplay)}</span>
      ${outputRaw != null ? '<span class="model-price-unit">/百万tokens</span>' : ''}
    </td>
  </tr>`;
}
...
export function renderModelPriceView(container, models, providerInfo = {}) {
  ...
  for (const model of pricedModels) {
    ...
    if (!providerSet.has(name)) {
      providerSet.set(name, { name, icon: model.providerIconUrl, vendor: model.vendor });
    }
  }
  const providers = [...providerSet.values()].sort((a, b) => a.name.localeCompare(b.name, 'zh-CN'));
  ...
  function getScopeModels() {
    ...
    return pricedModels.filter(m => providerDisplayName(m.vendor) === activeProvider);
  }
  ...
  function getBaseModels() {
    ...
    if (query) {
      list = list.filter(m =>
        ...
        || providerDisplayName(m.vendor).toLowerCase().includes(query)
      );
    }
    return list;
  }
  ...
  function renderModelTabs() {
    ...
    const allTab = `<button type="button" class="brand-tab${activeModel === 'all' ? ' is-active' : ''}" data-model-tab="all">
      <span>全部</span><span class="brand-count">${pricedModels.length}</span>
    </button>`;
    const tabs = latest.map(m => {
      ...
      return `<button type="button" class="brand-tab${activeModel === name ? ' is-active' : ''}" data-model-tab="${escapeHtml(name)}">
        ${providerIcon(m.vendor, m.providerIconUrl)}
  ...
  function renderProviderTabs() {
    ...
    const tabs = providers.map(p => {
      const count = pricedModels.filter(m => providerDisplayName(m.vendor) === p.name).length;
  ...
  function renderGroupRows(group, showAllGroups) {
    ...
    const header = canCollapse
      ? `<div class="plan-table-group-toggle" role="button" tabindex="0" data-model-group-toggle="${escapeHtml(group.name)}" aria-expanded="${isGroupExpanded ? 'true' : 'false'}" aria-label="${isGroupExpanded ? '收起' : '展开'}${escapeHtml(group.name)}模型">${headerInner}</div>`
  ...
  function renderFilterSummary(filteredCount, baseCount) {
    ...
    return `<div class="plan-table-filter-summary">
      <span class="plan-table-filter-chip">
        <span>${escapeHtml(SORT_KEYS[filterColumn].label)}</span>
        <strong>${escapeHtml(filterValue)}</strong>
      </span>
      <span class="plan-table-filter-count">${filteredCount} / ${baseCount} 个模型</span>
  ...
  function renderTable() {
    ...
    if (!baseModels.length) {
      return `<p class="model-price-empty">${searchQuery ? '暂无匹配的模型' : '暂无该厂商的模型价格数据'}</p>`;
    }
    ...
    <div class="model-price-table-wrap">
      <table class="model-price-table" role="grid" aria-label="模型价格对比表">
        ...
        <p class="model-price-footnote">价格单位：表内符号所示币种（¥ 人民币 / $ 美元，按厂商官方计价）/ 百万 tokens · 数据来源为各厂商官方定价页，真实价格请以官网为准</p>`;
  }
  ...
  function renderToolbar() {
    return `<div class="brand-filter-row model-price-toolbar">
      <div class="brand-tab-list">
        <button type="button" data-model-dimension="brand" class="brand-tab${activeDimension === 'brand' ? ' is-active' : ''}"><span>按品牌</span></button>
        <button type="button" data-model-dimension="model" class="brand-tab${activeDimension === 'model' ? ' is-active' : ''}"><span>按模型</span></button>
      </div>
    ...
    container.querySelectorAll('.model-price-th[data-sort-key]').forEach(th => {
      const handler = e => {
        ...
        if (sortKey === key) {
          ...
          sortKey = key;
          sortDir = SORT_KEYS[key]?.numeric ? 'asc' : 'asc';
        }
        renderContent();
      };
