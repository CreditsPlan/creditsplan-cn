import{a as ue,b as me,c as be,d as fe,e as ve,f as H,g as re,i as ge,j as he}from"./chunk.ACDYOHAY.js";import{a as ie}from"./chunk.EVJYX4CM.js";import{a as E,b as U,e as L,f as j,i as ce,j as de,k as ne,p as ae,q as pe}from"./chunk.D4H7VHJL.js";import{a as m}from"./chunk.XJPUQ4O3.js";var ye=2,Me=new Set(["claude-opus-4-8","claude-sonnet-4-6","claude-opus-4-7","claude-opus-4-6","claude-sonnet-4-5","claude-opus-4-5","claude-opus-4-1"]);function M(e){if(e==null||e==="")return null;let t=Number(e);return Number.isFinite(t)?t:null}function J(e,t){let n=M(e);return n==null?"\u5F85\u66F4\u65B0":`${t==="USD"?"$":"\xA5"}${n.toLocaleString("zh-CN",{maximumFractionDigits:4})}`}function $e(e){let t=M(e);return t==null?"\u2014":t>=1e6?`${(t/1e6).toLocaleString("zh-CN",{maximumFractionDigits:1})}M`:t>=1e3?`${(t/1e3).toLocaleString("zh-CN",{maximumFractionDigits:0})}K`:t.toLocaleString("zh-CN")}function T(e){return E[e]||e||"\u672A\u77E5"}var xe={};function Pe(e){let t=j(e,xe,E),n=String(t.seo_slug||"").trim(),l=String(t.seo_intro||"").trim(),o=String(t.icon_url||"").trim();return n&&l&&o?`/brands/${encodeURIComponent(n)}/`:""}function le(e,t){let n=U(e),l=t||n?.iconUrl||"";return H(l,T(e),"brand-icon brand-icon--tab")}function G(e){let t=String(e.raw?.lifecycle_status||"").trim().toLowerCase();if(t)return t==="legacy";if(String(e.vendor||"").trim().toLowerCase()!=="anthropic")return!1;let n=String(e.raw?.model_id||e.raw?.id||e.id||"").trim().toLowerCase().replace(/[._]/g,"-");return Me.has(n)}function oe(e){let t=String(e.raw?.release_date||"").trim();if(!t)return null;let n=Date.parse(t);return Number.isFinite(n)?n:null}var W={release:{numeric:!0,raw:oe},name:{label:"\u6A21\u578B",numeric:!1},provider:{label:"\u54C1\u724C",numeric:!1},context:{label:"\u4E0A\u4E0B\u6587",numeric:!0,raw:e=>M(e.raw?.context_length)},input:{label:"\u8F93\u5165\uFF08\u5B98\u65B9\u4EF7\uFF09",numeric:!0,raw:e=>M(e.raw?.input_price)},output:{label:"\u8F93\u51FA\uFF08\u5B98\u65B9\u4EF7\uFF09",numeric:!0,raw:e=>M(e.raw?.output_price)}};function Ce(e,t,n){let l=W[t];if(!l)return e;let o=[...e];return o.sort((c,d)=>{if(t==="release"){let u=Number(G(c))-Number(G(d));if(u!==0)return u}let f=0;if(l.numeric){let u=l.raw(c),b=l.raw(d);if(u==null||b==null)return u==null&&b==null?0:u==null?1:-1;f=u-b}else t==="name"?f=(c.modelName||"").localeCompare(d.modelName||"","zh-CN"):t==="provider"&&(f=T(c.vendor).localeCompare(T(d.vendor),"zh-CN"));return n==="desc"?-f:f}),o}function _e(e){let t=new Map;for(let n of e){let l=T(n.vendor),o=t.get(l);o||(o={name:l,vendor:n.vendor,icon:n.providerIconUrl,models:[]},t.set(l,o)),o.models.push(n)}return[...t.values()]}function Oe(e,t){let n=Number(G(t))-Number(G(e));if(n!==0)return n>0;let l=oe(e),o=oe(t);return l!=null&&o!=null?l>o:l!=null&&o==null}function Ae(e){let t=new Map;for(let n of e){let l=T(n.vendor),o=t.get(l);(!o||Oe(n,o))&&t.set(l,n)}return[...t.values()]}function Ne(e){let t=null;for(let n of e){let l=M(n.raw?.input_price);l==null||l<0||(!t||l<t.value)&&(t={value:l,currency:n.raw?.currency})}return t}function De(e){let t=Ne(e.models),n=[];if(t){let l=t.currency==="USD"?"$":"\xA5";n.push(`\u8F93\u5165 ${l}${t.value.toLocaleString("zh-CN",{maximumFractionDigits:4})} \u8D77`)}return n.push(`${e.models.length} \u4E2A\u6A21\u578B`),`<span class="plan-table-group-summary">${m(n.join(" \xB7 "))}</span>`}var Se={name:e=>String(e.modelName||"").trim()||"\u2014",provider:e=>T(e.vendor),context:e=>$e(e.raw?.context_length),input:e=>J(e.raw?.input_price,e.raw?.currency),output:e=>J(e.raw?.output_price,e.raw?.currency)},we=new Set(["\u2014","\u5F85\u66F4\u65B0"]),Fe=new Intl.Collator("zh-CN",{numeric:!0,sensitivity:"base"});function ke(e,t){let n=Se[t];return n?String(n(e)||"").trim()||"\u2014":""}function qe(e,t){let n=new Map;for(let l of e){let o=ke(l,t);n.set(o,(n.get(o)||0)+1)}return Array.from(n.entries()).map(([l,o])=>({value:l,count:o})).sort((l,o)=>{let c=we.has(l.value),d=we.has(o.value);return c!==d?c?1:-1:Fe.compare(l.value,o.value)})}function Be(e,t,n){return e!==t?'<svg class="model-price-sort-icon model-price-sort-icon--idle" viewBox="0 0 12 12" aria-hidden="true"><path d="M6 2l2.5 3h-5zM6 10l-2.5-3h5z" fill="currentColor"/></svg>':`<svg class="model-price-sort-icon" viewBox="0 0 12 12" aria-hidden="true"><path d="${n==="asc"?"M6 2l3 4H3z":"M6 10L3 6h6z"}" fill="currentColor"/></svg>`}function Ie(e,t,n){let l=W[e],o=t.column===e&&!!t.value,c=qe(n,e);return`<button type="button" class="plan-column-filter-trigger model-price-filter-trigger${o?" is-active":""}" data-model-filter-column="${e}" aria-haspopup="menu" aria-expanded="false" title="\u7B5B\u9009${m(l.label)}">
    ...
    <td class="model-price-td model-price-td--price">
  ...
  </tr>`}function Ee(e,t,n={}){xe=n||{};let l=t.filter(a=>M(a.raw?.input_price)!=null||M(a.raw?.output_price)!=null),o=new Map;for(let a of l){let s=T(a.vendor);o.has(s)||o.set(s,{name:s,icon:a.providerIconUrl,vendor:a.vendor})}let c=[...o.values()].sort((a,s)=>a.name.localeCompare(s.name,"zh-CN")),d="all",f="release",u="desc",b="brand",g="all",w="",x="",h="",S=new Set;function _(){return!!(x&&h&&Se[x])}function D(){return b==="model"?g==="all"?l:l.filter(a=>(a.modelName||"")===g):d==="all"?l:l.filter(a=>T(a.vendor)===d)}function R(){let a=D(),s=w.toLowerCase();return s&&(a=a.filter(i=>String(i.modelName||"").toLowerCase().includes(s)||T(i.vendor).toLowerCase().includes(s))),a}function Y(a){let s=a;return _()&&(s=s.filter(i=>ke(i,x)===h)),Ce(s,f,u)}function C(){return b==="model"?V():F()}function V(){let a=Ae(l).sort((p,$)=>(p.modelName||"").localeCompare($.modelName||"","zh-CN")),s=`<button type="button" class="brand-tab${g==="all"?" is-active":""}" data-model-tab="all">
      <span>\u5168\u90E8</span><span class="brand-count">${l.length}</span>
        ...
        <span>${m($)}</span>
      </button>`}).join("");return`<div class="brand-tab-list model-price-tabs">${s}<span class="brand-divider"></span>${i}</div>`}function F(){let a=`<button type="button" class="brand-tab${d==="all"?" is-active":""}" data-provider="all">
      <span>\u5168\u90E8</span><span class="brand-count">${l.length}</span>
    </button>`,s=c.map(i=>{let p=l.filter($=>T($.vendor)===i.name).length;return`<button type="button" class="brand-tab${d===i.name?" is-active":""}" data-provider="${m(i.name)}">
        ${le(i.vendor,i.icon)}
        <span>${m(i.name)}</span>
        <span class="brand-count">${p}</span>
      </button>`}).join("");return`<div class="brand-tab-list model-price-tabs">${a}<span class="brand-divider"></span>${s}</div>`}function B(a,s){let i=!s&&a.models.length>ye,p=s||!i||S.has(a.name),$=p?a.models:a.models.slice(0,ye),te=Pe(a.vendor),r=`${H(a.icon||U(a.vendor)?.iconUrl||"",a.name,"brand-icon brand-icon--section")}
            <span class="text-sm font-bold text-brand-800 dark:text-brand-200">${m(a.name)}</span>`,v=`
            ${te?`<a href="${m(te)}" class="plan-table-group-brand">${r}</a>`:r}
      ...
      <tr class="border-y border-slate-200 dark:border-slate-700">
        <td colspan="5" class="bg-brand-50/70 p-0 dark:bg-brand-950/20">
          ${i?`<div class="plan-table-group-toggle" role="button" tabindex="0" data-model-group-toggle="${m(a.name)}" aria-expanded="${p?"true":"false"}" aria-label="${p?"\u6536\u8D77":"\u5C55\u5F00"}${m(a.name)}\u6A21\u578B">${v}</div>`:`<div class="plan-table-group-toggle plan-table-group-toggle--static">${v}</div>`}
        </td>
      </tr>
      ${$.map(He).join("")}`}function N(a,s){return _()?`<div class="plan-table-filter-summary">
  ...
  `,Le();let x=y.codingPlanOverview.querySelector(".plans-workbench"),h=y.codingPlanOverview.querySelector("#brandFilterBar"),S=y.codingPlanOverview.querySelector("#brandTabs"),_=y.codingPlanOverview.querySelector("#modelTabs"),D=y.codingPlanOverview.querySelector("#brandDetail");Ye(x);let R=y.codingPlanOverview.querySelector("#planAdvisorFab"),Y=ge({plans:o,providerInfo:t,modelCatalog:n,fab:R});Y&&location.hash==="#advisor"&&Y.open();let C=o,V="all",F="brand";Qe(y.codingPlanOverview,()=>fe(C),t,()=>F,()=>l);let B="",N=new Set,z=h.querySelector("[data-plan-available-toggle]"),ee=()=>{if(!z)return;let r=ue();z.classList.toggle("is-active",r),z.setAttribute("aria-pressed",String(r))},q=()=>{if(ee(),F==="pricing"){Ee(D,l,t);return}D.innerHTML=re(C,B,t,N,V!=="all")},k=()=>{be(),B="",N.clear()},K=()=>{N.clear(),q()};ve(D,()=>C,K,r=>{B=B===r?"":r,q()});let I=r=>{N.has(r)?N.delete(r):N.add(r),q()};D.addEventListener("click",r=>{if(r.target.closest("a"))return;let v=r.target.closest("[data-plan-group-toggle]");v&&I(v.dataset.planGroupToggle)}),D.addEventListener("keydown",r=>{if(r.key!=="Enter"&&r.key!==" ")return;let v=r.target.closest("[data-plan-group-toggle]");!v||v.tagName==="BUTTON"||r.target.closest("a")||(r.preventDefault(),I(v.dataset.planGroupToggle))});let Q=()=>{[S,_].forEach(r=>{r.querySelectorAll(".brand-tab").forEach(v=>v.classList.remove("is-active"))})},a=r=>{r==="all"?C=o:r==="free"?C=ae(o):c.has(r)?C=c.get(r).plans:f.has(r)&&(C=f.get(r).plans)},s=r=>{let v=r==="pricing"?Z.pricing:Z.plans,O=y.codingPlanOverview.querySelector("#codingPlanTitle"),P=y.codingPlanOverview.querySelector("#workbenchSummary"),A=y.codingPlanOverview.querySelector("#workbenchStats");if(O&&(O.textContent=v.title),P&&(P.textContent=v.summary),!!A)if(r==="pricing"){let X=l.filter(Ve),Te=new Set(X.map(se=>E[se.vendor]||se.vendor)).size;A.innerHTML=`<span>${X.length} \u4E2A\u6A21\u578B</span><span>${Te} \u4E2A\u5382\u5546</span>`}else A.innerHTML=w},i=r=>{r!==F&&(F=r,h.querySelectorAll("[data-dimension]").forEach(v=>{v.classList.toggle("is-active",v.dataset.dimension===r)}),S.hidden=r!=="brand",_.hidden=r!=="model",p&&(p.placeholder=r==="brand"?"\u641C\u7D22\u54C1\u724C\u2026":"\u641C\u7D22\u6A21\u578B\u2026"),k(),V="all",C=o,Q(),r==="pricing"?h.hidden=!0:(h.hidden=!1,(r==="brand"?S:_).querySelector('[data-brand="all"]')?.classList.add("is-active")),R&&(R.hidden=r==="pricing"),p&&(p.value=""),$(),s(r),Je(r),q())},p=y.codingPlanOverview.querySelector("#brandSearchInput"),$=()=>{let r=(p?.value||"").trim().toLowerCase(),v=F==="brand"?S:_;v.querySelectorAll(".brand-tab[data-brand]").forEach(P=>{let A=P.dataset.brand;if(A==="all"||A==="free"){P.hidden=!1;return}let X=(P.dataset.brandLabel||"").toLowerCase();P.hidden=r?!X.includes(r):!1});let O=v.querySelector(".brand-divider");O&&(O.hidden=!1)};p?.addEventListener("input",$),h.addEventListener("click",r=>{if(r.target.closest("[data-plan-available-toggle]")){me(),K();return}let O=r.target.closest("[data-dimension]");if(O){i(O.dataset.dimension);return}let P=r.target.closest(".brand-tab");if(!P||!S.contains(P)&&!_.contains(P))return;let A=P.dataset.brand;k(),V=A,Q(),P.classList.add("is-active"),a(A),q()}),((globalThis.location?.pathname||"").replace(/\/+$/,"")||"/")==="/model"&&i("pricing")}function Je(e){if(typeof globalThis.history?.replaceState=="function")try{let t=new URL(globalThis.location.href),n=e==="pricing"?"/model":"/";globalThis.history.replaceState(null,"",`${n}${t.searchParams.toString()?`?${t.searchParams.toString()}`:""}${t.hash}`)}catch{}}function Ze(e){if(!y.codingPlanOverview)return;let t=e==="backend"?"\u672C\u5730\u6570\u636E\u5E93\u63A5\u53E3 /api/models \u5F53\u524D\u4E0D\u53EF\u7528\uFF0C\u8BF7\u786E\u8BA4 public-api \u5DF2\u542F\u52A8\u5E76\u8FDE\u63A5\u6570\u636E\u5E93\u3002":"\u90E8\u7F72\u5305\u4E2D\u7684 data.json \u5F53\u524D\u4E0D\u53EF\u7528\uFF0C\u8BF7\u91CD\u65B0\u5BFC\u51FA\u6570\u636E\u5E93\u5FEB\u7167\u5E76\u90E8\u7F72\u3002";y.codingPlanOverview.innerHTML=`
    <section class="plans-workbench" aria-labelledby="codingPlanTitle">
      <div class="workbench-head">
        <div>
  ...
  `,Le()}async function et(){ie();let e=await he();if(e.dataUnavailable){Ze(e.source);return}Xe(e.plans,e.providerInfo||{},e.modelCatalog||[],e.models||[])}et();
