...
async function loadItems(append = false) {
  ...
  if (!append) {
    ...
    if (els.loadMoreBtn) els.loadMoreBtn.textContent = '加载中...';
  }
  ...
  try {
    ...
    if (els.errorMessage) els.errorMessage.textContent = 'AI 动态服务暂时不可用，请稍后再试。';
    showState('error');
  } finally {
    state.loading = false;
    if (els.loadMoreBtn) els.loadMoreBtn.textContent = '加载更多';
  }
}
...
function renderTimelineItem(item) {
  ...
  const selectedBadge = selected
    ? `<span class="aihot-selected-badge">精选</span>`
    : '';
  ...
  const reasonHtml = reason
    ? `<div class="aihot-card-reason"><span class="aihot-reason-label">推荐理由</span>${escapeHtml(reason)}</div>`
    : '';
...
async function loadDailyView() {
  ...
  try {
    ...
    if (els.dailyErrorMessage) els.dailyErrorMessage.textContent = 'AI 日报服务暂时不可用，请稍后再试。';
...
function populateDailySelect() {
  ...
  if (!state.dailyDates.length) {
    els.dailyDateSelect.innerHTML = '<option value="">日报尚未生成</option>';
    return;
  }
...
function renderDailySections(sections) {
  ...
  const filtered = sections.filter(s => {
    ...
    return !text.includes('international') && !text.includes('国际');
  });
